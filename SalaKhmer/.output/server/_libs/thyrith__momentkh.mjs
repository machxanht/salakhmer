...
//#region node_modules/@thyrith/momentkh/momentkh.js
var require_momentkh = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(root, factory) {
		...
		*/
		...
		exports$1.constants = exports$1.DayOfWeek = exports$1.Sak = exports$1.AnimalYear = exports$1.MonthIndex = exports$1.MoonPhase = void 0;
		exports$1.fromGregorian = fromGregorian;
		exports$1.fromKhmer = fromKhmer;
		exports$1.getNewYear = getNewYear;
		exports$1.format = format;
		exports$1.fromDate = fromDate;
		exports$1.toDate = toDate;
		var MoonPhase;
		(function(MoonPhase) {
			MoonPhase[MoonPhase["Waxing"] = 0] = "Waxing";
			MoonPhase[MoonPhase["Waning"] = 1] = "Waning";
		})(MoonPhase || (exports$1.MoonPhase = MoonPhase = {}));
		var MonthIndex;
		(function(MonthIndex) {
			...
			MonthIndex[MonthIndex["Tutiyasadh"] = 13] = "Tutiyasadh";
		})(MonthIndex || (exports$1.MonthIndex = MonthIndex = {}));
		var AnimalYear;
		(function(AnimalYear) {
			...
			AnimalYear[AnimalYear["Kor"] = 11] = "Kor";
		})(AnimalYear || (exports$1.AnimalYear = AnimalYear = {}));
		var Sak;
		(function(Sak) {
			...
			Sak[Sak["NappaSak"] = 9] = "NappaSak";
		})(Sak || (exports$1.Sak = Sak = {}));
		var DayOfWeek;
		(function(DayOfWeek) {
		...
		})(DayOfWeek || (exports$1.DayOfWeek = DayOfWeek = {}));
		const LunarMonths = {
			"មិគសិរ": 0,
			"បុស្ស": 1,
			"មាឃ": 2,
		...
		function getDayOfWeek(year, month, day) {
			return (gregorianToJulianDay(year, month, day) + 1) % 7;
		}
		/**
		...
		*/
		function validateGregorianDate(year, month, day, hour = 0, minute = 0, second = 0) {
			if (typeof year !== "number" || isNaN(year)) throw new Error(`Invalid year: ${year}. Year must be a valid number.`);
			if (typeof month !== "number" || isNaN(month)) throw new Error(`Invalid month: ${month}. Month must be a valid number.`);
			if (typeof day !== "number" || isNaN(day)) throw new Error(`Invalid day: ${day}. Day must be a valid number.`);
			if (typeof hour !== "number" || isNaN(hour)) throw new Error(`Invalid hour: ${hour}. Hour must be a valid number.`);
			if (typeof minute !== "number" || isNaN(minute)) throw new Error(`Invalid minute: ${minute}. Minute must be a valid number.`);
			if (typeof second !== "number" || isNaN(second)) throw new Error(`Invalid second: ${second}. Second must be a valid number.`);
			if (month < 1 || month > 12) throw new Error(`Invalid month: ${month}. Month must be between 1 and 12.`);
			...
			if (day < 1 || day > daysInMonth) throw new Error(`Invalid day: ${day}. ${[
				...
				"November",
				"December"
			][month - 1]} ${year} has ${daysInMonth} days.`);
			if (hour < 0 || hour > 23) throw new Error(`Invalid hour: ${hour}. Hour must be between 0 and 23.`);
			if (minute < 0 || minute > 59) throw new Error(`Invalid minute: ${minute}. Minute must be between 0 and 59.`);
			if (second < 0 || second > 59) throw new Error(`Invalid second: ${second}. Second must be between 0 and 59.`);
		}
		/**
		...
		*/
		function validateKhmerDate(day, moonPhase, monthIndex, beYear) {
			if (typeof day !== "number" || isNaN(day)) throw new Error(`Invalid day: ${day}. Day must be a valid number.`);
			if (typeof moonPhase !== "number" || isNaN(moonPhase)) throw new Error(`Invalid moonPhase: ${moonPhase}. moonPhase must be a valid number.`);
			if (typeof monthIndex !== "number" || isNaN(monthIndex)) throw new Error(`Invalid monthIndex: ${monthIndex}. monthIndex must be a valid number.`);
			if (typeof beYear !== "number" || isNaN(beYear)) throw new Error(`Invalid beYear: ${beYear}. beYear must be a valid number.`);
			if (day < 1 || day > 15) throw new Error(`Invalid day: ${day}. Lunar day must be between 1 and 15.`);
			const moonPhaseNum = typeof moonPhase === "number" ? moonPhase : moonPhase;
			if (moonPhaseNum !== 0 && moonPhaseNum !== 1) throw new Error(`Invalid moonPhase: ${moonPhase}. moonPhase must be 0 (Waxing/កើត) or 1 (Waning/រោច).`);
			const monthIndexNum = typeof monthIndex === "number" ? monthIndex : monthIndex;
			if (monthIndexNum < 0 || monthIndexNum > 13) throw new Error(`Invalid monthIndex: ${monthIndex}. monthIndex must be between 0 and 13.`);
			if (beYear < 2e3 || beYear > 3e3) throw new Error(`Invalid beYear: ${beYear}. beYear must be between 2000 and 3000.`);
		}
		/**
		...
		*/
		function validateDateObject(date) {
			if (!(date instanceof Date)) throw new Error("Invalid input: Expected a Date object.");
			if (isNaN(date.getTime())) throw new Error("Invalid Date object: Date is not a valid date.");
		}
		...
		function isKhmerLeapDayByCalculation(beYear) {
			...
			else if (isSolarLeap) return avoman < 127;
			else if (avoman === 137 && getAvoman(beYear + 1) === 0) return false;
			else if (avoman < 138) return true;
			return false;
		}
		function isKhmerLeapMonth(beYear) {
			const bodithey = getBodithey(beYear);
			const boditheyNextYear = getBodithey(beYear + 1);
			if (bodithey === 25 && boditheyNextYear === 5) return false;
			return bodithey === 24 && boditheyNextYear === 6 || bodithey >= 25 || bodithey < 6;
		}
		function getLeapType(beYear) {
			if (isKhmerLeapMonth(beYear)) return 1;
			else if (isKhmerLeapDayByCalculation(beYear)) return 2;
			else if (isKhmerLeapMonth(beYear - 1)) {
				let previousYear = beYear - 1;
				while (true) {
					if (isKhmerLeapDayByCalculation(previousYear)) return 2;
					previousYear -= 1;
					if (!isKhmerLeapMonth(previousYear)) return 0;
				}
			}
			return 0;
		}
		function getNumberOfDaysInKhmerMonth(monthIndex, beYear) {
			...
			if (idx === MonthIndex.Jesth && leapType === 2) return 30;
			if (idx === MonthIndex.Pathamasadh || idx === MonthIndex.Tutiyasadh) return leapType === 1 ? 30 : 0;
			return idx % 2 === 0 ? 29 : 30;
		}
		function getNumberOfDaysInKhmerYear(beYear) {
			const leapType = getLeapType(beYear);
			if (leapType === 1) return 384;
			if (leapType === 2) return 355;
			return 354;
		}
		function nextMonthOf(monthIndex, beYear) {
			const leapType = getLeapType(beYear);
			const idx = typeof monthIndex === "number" ? monthIndex : monthIndex;
			if (idx === MonthIndex.Jesth && leapType === 1) return MonthIndex.Pathamasadh;
			if (idx === MonthIndex.Kadeuk) return MonthIndex.Migasir;
			if (idx === MonthIndex.Pathamasadh) return MonthIndex.Tutiyasadh;
			if (idx === MonthIndex.Tutiyasadh) return MonthIndex.Srap;
			return idx + 1;
		}
		function previousMonthOf(monthIndex, beYear) {
			const leapType = getLeapType(beYear);
			const idx = typeof monthIndex === "number" ? monthIndex : monthIndex;
			if (idx === MonthIndex.Migasir) return MonthIndex.Kadeuk;
			if (idx === MonthIndex.Srap && leapType === 1) return MonthIndex.Tutiyasadh;
			if (idx === MonthIndex.Tutiyasadh) return MonthIndex.Pathamasadh;
			if (idx === MonthIndex.Pathamasadh) return MonthIndex.Jesth;
			return idx - 1;
		}
		...
		function getSunInfo(jsYear, sotin) {
			const infoOfPrevYear = { kromathopol: getKromthupulJs(jsYear - 1) };
			const r2 = 800 * sotin + infoOfPrevYear.kromathopol;
			const reasey = Math.floor(r2 / 24350);
			const r3 = r2 % 24350;
			const angsar = Math.floor(r3 / 811);
			const r4 = r3 % 811;
			const libda = Math.floor(r4 / 14) - 3;
			const sunAverageAsLibda = 1800 * reasey + 60 * angsar + libda;
			const s1 = 4800;
			let leftOver = sunAverageAsLibda - s1;
			if (sunAverageAsLibda < s1) leftOver += 21600;
			const kaen = Math.floor(leftOver / 1800);
			let rs = -1;
			if ([
				0,
				1,
				2
			].includes(kaen)) rs = kaen;
			else if ([
				3,
				4,
				5
			].includes(kaen)) rs = 10800 - leftOver;
			else if ([
				6,
				7,
				8
			].includes(kaen)) rs = leftOver - 10800;
			else if ([
				9,
				10,
				11
			].includes(kaen)) rs = 21600 - leftOver;
			const lastLeftOver = {
				reasey: Math.floor(rs / 1800),
				angsar: Math.floor(rs % 1800 / 60),
				libda: rs % 60
			};
			let khan, pouichalip;
			...
			const pholAsLibda = Math.floor(pouichalip * chhayaSun.multiplicity / 900) + chhayaSun.chhaya;
			const sunInaugurationAsLibda = kaen <= 5 ? sunAverageAsLibda - pholAsLibda : sunAverageAsLibda + pholAsLibda;
			return {
				sunInaugurationAsLibda,
				reasey: Math.floor(sunInaugurationAsLibda / 1800),
				angsar: Math.floor(sunInaugurationAsLibda % 1800 / 60),
				libda: sunInaugurationAsLibda % 60
			};
		}
		function getNewYearInfo(jsYear) {
			const newYearsDaySotins = (has366Days(jsYear - 1) ? [
				363,
				364,
				365,
				366
				...
				364,
				365
			]).map((sotin) => getSunInfo(jsYear, sotin));
			let timeOfNewYear = {
				hour: 0,
				minute: 0
			};
			for (const sotin of newYearsDaySotins) if (sotin.angsar === 0) {
				const minutes = 1440 - sotin.libda * 24;
				timeOfNewYear = {
		...
		class KhmerDate {
			...
			getDayNumber() {
				if (this.moonPhase === MoonPhase.Waxing) return this.day - 1;
				else return 15 + (this.day - 1);
			}
				...
				while (remaining > 0) {
					...
					if (remaining <= daysLeftInMonth) {
						...
						if (result.monthIndex === MonthIndex.Pisakh) {
							if (result.moonPhase === MoonPhase.Waxing && newDay.moonPhase === MoonPhase.Waning) newBeYear++;
						}
						...
						result = new KhmerDate(1, MoonPhase.Waxing, nextMonth, newBeYear);
					}
				}
				return result;
			}
			subtractDays(count) {
				...
				while (remaining > 0) {
					...
					if (remaining <= currentDayNum) {
						...
						let newBeYear = result.beYear;
						if (result.monthIndex === MonthIndex.Pisakh) {
							if (result.moonPhase === MoonPhase.Waning && newDay.moonPhase === MoonPhase.Waxing) newBeYear--;
						}
						result = new KhmerDate(newDay.day, newDay.moonPhase, result.monthIndex, newBeYear);
						remaining = 0;
					} else {
		...
		function getMaybeBEYear(year, month) {
			if (month <= 4) return year + 543;
			else return year + 544;
		}
		...
		*/
		function getPisakhaBochea(year, isSearching = false) {
			if (visakhaBocheaCache[year]) return visakhaBocheaCache[year];
		...
		function gregorianToKhmerInternal(year, month, day, hour = 0, minute = 0, second = 0, isSearching = false) {
			...
			*/
			...
			let khmerMonth = 1;
			let khmerDayNumber = 0;
			let diffDays = targetJdn - epochJdn;
			if (diffDays > 0) while (true) {
				const epochGreg = julianDayToGregorian(epochJdn);
				const daysInNextYear = getNumberOfDaysInKhmerYear(getMaybeBEYear(epochGreg.year + 1, epochGreg.month));
				if (diffDays > daysInNextYear) {
					diffDays -= daysInNextYear;
					epochJdn += daysInNextYear;
				} else break;
			}
			else if (diffDays < 0) while (diffDays < 0) {
				const epochGreg = julianDayToGregorian(epochJdn);
				const daysInCurrentYear = getNumberOfDaysInKhmerYear(getMaybeBEYear(epochGreg.year, epochGreg.month));
			...
			let beYear;
			if (isSearching) beYear = month <= 4 ? year + 543 : year + 544;
			else if (new Date(year, month - 1, day, hour, minute, second).getTime() >= getPisakhaBochea(year)) beYear = year + 544;
			else beYear = year + 543;
			...
			if (!isSearching) {
				...
				if (inputTimestamp >= newYearInfo.newYearMoment.getTime() && inputTimestamp <= visakhaBocheaTimestamp) animalYearIndex = (animalYearIndex + 1) % 12;
				if (inputTimestamp >= newYearInfo.lerngSakMoment.getTime() && inputTimestamp <= visakhaBocheaTimestamp) jsYear++;
			}
		...
		function khmerToGregorian(day, moonPhase, monthIndex, beYear) {
			...
			let candidates = [];
			for (let year = startYear; year <= endYear; year++) for (let month = 1; month <= 12; month++) {
				const daysInMonth = getDaysInGregorianMonth(year, month);
				for (let gDay = 1; gDay <= daysInMonth; gDay++) {
					const timesToCheck = monthIndexNum === MonthIndex.Pisakh && (day === 15 && moonPhaseNum === MoonPhase.Waxing || day === 1 && moonPhaseNum === MoonPhase.Waning) ? [
						0,
						6,
						12,
						18,
					...
					for (const hour of timesToCheck) {
						...
						if (khmerResult.khmer.beYear === beYear && khmerResult.khmer.monthIndex === monthIndexNum && khmerResult.khmer.day === day && khmerResult.khmer.moonPhase === moonPhaseNum) {
							candidates.push({
								year,
								month,
								day: gDay
			...
			if (candidates.length === 0) throw new Error(`Could not find Gregorian date for Khmer date: ${day} ${moonPhaseNum === MoonPhase.Waxing ? "កើត" : "រោច"} month ${monthIndexNum} BE ${beYear}`);
			if (candidates.length > 1) {
				...
				if (closestCandidates.length === 1) return closestCandidates[0];
				const noonMatches = closestCandidates.filter((c) => {
					const noonCheck = gregorianToKhmerInternal(c.year, c.month, c.day, 12, 0, 0, false);
					return noonCheck.khmer.beYear === beYear && noonCheck.khmer.monthIndex === monthIndexNum && noonCheck.khmer.day === day && noonCheck.khmer.moonPhase === moonPhaseNum;
				});
				if (noonMatches.length > 0) return noonMatches[0];
				return closestCandidates[0];
			}
			return candidates[0];
		}
		function getNewYearFullInfo(ceYear) {
			if (newYearInfoCache[ceYear]) return newYearInfoCache[ceYear];
			...
			if (isAthikameasPrev && isChantrathimeasPrev) bodithey = (bodithey + 1) % 30;
			...
			const epochLerngSakGreg = {
				year: ceYear,
				month: 4,
				day: 17
			};
			const khEpoch = gregorianToKhmerInternal(ceYear, 4, 17, 12, 0, 0, true)._khmerDateObj;
			const daysToSubtract = (khEpoch.monthIndex - 4) * 29 + khEpoch.getDayNumber() - ((lunarDateLerngSak.month - 4) * 29 + lunarDateLerngSak.day) + numberNewYearDay - 1;
			let newYearJdn = gregorianToJulianDay(epochLerngSakGreg.year, epochLerngSakGreg.month, epochLerngSakGreg.day) - daysToSubtract;
			if (khNewYearMoments[ceYear]) {
				const [datePart, timePart] = khNewYearMoments[ceYear].split(" ");
				const [d, m, y] = datePart.split("-").map(Number);
				const [hr, min] = timePart.split(":").map(Number);
				newYearInfo.timeOfNewYear = {
					hour: hr,
					minute: min
				};
			...
			const lerngSakDate = julianDayToGregorian(newYearJdn + numberNewYearDay - 1);
			const result = {
				newYearMoment,
				lerngSakMoment: new Date(lerngSakDate.year, lerngSakDate.month - 1, lerngSakDate.day, 0, 0, 0),
				newYearInfo: {
		...
		function getKhmerNewYear(ceYear) {
			return getNewYearFullInfo(ceYear).newYearInfo;
		}
		function formatKhmer(khmerData, formatString) {
			...
			return formatString.replace(regex, (match, escaped, token) => {
