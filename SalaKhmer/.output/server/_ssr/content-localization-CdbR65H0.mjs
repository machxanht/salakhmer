import { d as EXPANDED_VI_TEXT } from "./router-JcPmpmb6.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/content-localization-CdbR65H0.js
function isCategoryAccessible(role, category) {
...
var viPlaces = {
	...
	Pharmacy: "nhà thuốc",
	Restaurant: "nhà hàng",
	"Central Market": "Chợ Trung tâm",
	"Orussey Market": "Chợ Orussey",
	"Night Market": "chợ đêm",
	Riverside: "bờ sông"
};
function viDynamicText(value) {
	...
	const scenario = {
		"Meeting someone": "Gặp gỡ người mới",
		"At home": "Ở nhà",
		"Counting money": "Đếm tiền",
		"Buying food": "Mua đồ ăn",
		"At school": "Ở trường",
		"Morning plans": "Kế hoạch buổi sáng",
		"Choosing colours": "Chọn màu sắc",
		"Polite answers": "Câu trả lời lịch sự",
		"Market question": "Hỏi ở chợ",
		"Daily essentials": "Nhu cầu hằng ngày"
	};
	...
	match = value.match(/^Family Discussion$/);
	if (match) return "Trò chuyện về gia đình";
	match = value.match(/^Pricing Query for (.+)$/);
	if (match) return `Hỏi giá ${match[1]}`;
	match = value.match(/^Market Shopping for (.+)$/);
	if (match) return `Mua ${match[1]} ở chợ`;
	match = value.match(/^Ordering (.+) at Restaurant$/);
	if (match) return `Gọi ${match[1]} tại nhà hàng`;
}
/**
...
*/
function localizeLegacyText(value, locale) {
	if (!value || locale === "en") return value ?? "";
	if (locale === "vi") return EXPANDED_VI_TEXT[value] ?? viText[value] ?? viDynamicText(value) ?? value;
	return value;
}
function localizeLegacyLesson(lesson, locale) {
...
//#endregion
export { localizeLegacyLesson as n, localizeLegacyText as r, isCategoryAccessible as t };
